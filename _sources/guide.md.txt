...
**personio-py** works with Python 3.10 or higher. Python 2 is not supported, as well as Python 3.9 or lower. Using a recent version of Python allows us to write modern Python code without ugly compatibility layers. Also, we can avoid some dependencies that used to be mandatory, like `python-dateutil`, now that Python comes with more useful date & time handling out of the box.

The only dependency of personio-py is the [`requests`](https://pypi.org/project/requests/) library, which handles all API calls.

### Pip

The easiest way to install personio-py is using [pip](https://pip.pypa.io/en/stable/), which should be already installed when you're using Python 3.10 or higher (which is required to use personio-py).
...
Now that personio-py is installed, we can start making requests. All requests are made through the {py:class}`Personio <personio_py.Personio>` class, which also handles authentication.
